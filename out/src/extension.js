...
const toolProcessor_1 = require("./utils/toolProcessor");
class InteractiveToolViewProvider {
    constructor(_context) {
        this._activeProcesses = new Map(); // sequenceId -> processId
        ...
        this._toolProcessor = toolProcessor_1.ToolProcessor.getInstance({
            toolsDir: toolsDir
        });
    }
    ...
    handleWebviewMessage(message, webview) {
        switch (message.command) {
            case 'executeCommand': {
                // 处理从Webview发送的终端命令
                logger_1.Logger.debug(`Received terminal command from webview: ${message.text || ''}`);
            ...
            case 'executeTool': {
                ...
                if (!message.toolName || !message.text || !message.sequenceId) {
                    logger_1.Logger.error('Invalid tool execution request: missing required parameters');
                    void webview.postMessage({
                        command: 'toolError',
                ...
                logger_1.Logger.debug(`Received tool execution request: ${message.toolName}, command: ${message.text}, sequenceId: ${message.sequenceId}`);
                // 执行工具
                const processId = this._toolProcessor.executeTool(message.toolName, message.text, message.sequenceId, (response) => {
                    // 转发响应到Webview
                    void webview.postMessage({
                    ...
                    // 处理执行错误
                    logger_1.Logger.error(`Tool execution error: ${error.message}`);
                    void webview.postMessage({
                        command: 'toolError',
                        error: error.message,
                        sequenceId: message.sequenceId
                    ...
                    // 执行完成回调
                    logger_1.Logger.debug(`Tool execution completed: ${message.toolName}, sequenceId: ${message.sequenceId}`);
            ...
            case 'cancelExecution': {
                ...
                if (message.sequenceId) {
                    ...
                    if (processId) {
                        logger_1.Logger.debug(`Cancelling execution for sequenceId: ${message.sequenceId}`);
...
function activate(context) {
    ...
    toolProcessor_1.ToolProcessor.getInstance({
        toolsDir: toolsDir
    });
    // 创建TerminalManager实例
    const terminalManager = new terminalManager_1.TerminalManager();
