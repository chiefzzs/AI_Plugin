...
class WebviewManager {
    ...
    createOrShowWebview(context) {
        ...
        if (this.webviewPanel) {
            ...
            logger_1.Logger.debug('Revealed existing webview panel');
            return;
        }
        // 创建新的Webview
        this.webviewPanel = vscode.window.createWebviewPanel('interactiveTool', 'Interactive Tool', column || vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [vscode.Uri.file(context.asAbsolutePath('static/dist'))]
        });
        logger_1.Logger.debug('Created new webview panel');
        // 设置Webview内容
        this.webviewPanel.webview.html = this.getWebviewContent(context);
        // 处理Webview关闭事件
        this.webviewPanel.onDidDispose(() => {
            this.webviewPanel = undefined;
            logger_1.Logger.debug('Webview panel disposed');
        }, null, context.subscriptions);
        // 设置消息接收处理
        this.webviewPanel.webview.onDidReceiveMessage((message) => this.handleWebviewMessage(message), null, context.subscriptions);
    }
    /**
    ...
    getWebviewContent(context) {
        ...
        try {
            ...
            logger_1.Logger.error('Failed to load HTML template:', error);
    ...
    handleWebviewMessage(message) {
        // 实现消息处理逻辑
        logger_1.Logger.debug('Received message from webview:', message);
    }
    /**
     * 向Webview发送消息
    ...
    sendMessageToWebview(message) {
        if (this.webviewPanel) {
            ...
            logger_1.Logger.debug('Sent message to webview:', message);
        }
        else {
            logger_1.Logger.warn('No active webview panel to send message');
        }
    }
    /**
     * 检查Webview是否存在
