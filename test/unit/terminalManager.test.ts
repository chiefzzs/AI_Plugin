...
class MockTerminal {
  ...
  processId: Thenable<number> = Promise.resolve(123);
  exitStatus: any | undefined = undefined;
...
// 我们已经在setup.js中全局mock了vscode模块，所以这里不需要再次mock
// 声明全局的vscode对象以避免TypeScript编译错误
declare const vscode: any;

describe('TerminalManager', () => {
  let terminalManager: TerminalManager;
  let mockTerminals: Map<string, MockTerminal>;
  let mockTerminalDataListeners: ((e: any) => void)[];
  ...
  beforeEach(() => {
    ...
    jest.spyOn(vscode.window, 'createTerminal').mockImplementation((options: any) => {
  ...
  describe('executeCommand方法', () => {
    it('应该在指定终端执行命令', async () => {
      ...
      const terminal = terminalManager.createTerminal(terminalName) as unknown as MockTerminal;
  ...
  describe('disposeAllTerminals方法', () => {
    it('应该销毁所有已创建的终端', () => {
      ...
      // 创建多个终端
      terminalNames.forEach(name => {
        terminals.push(terminalManager.createTerminal(name) as unknown as MockTerminal);
      });
